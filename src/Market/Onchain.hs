...
import Market.Types    (RoyAddr(..),NFTSale(..), SaleAction(..), MarketParams(..))
...
mkBuyValidator mp nfts r ctx = case r of
    Buy     -> checkFee (nPrice nfts) &&
               ...
               checkSellerOut (nSeller nfts) ( sumRoyalty $ rAddr nfts) (nPrice nfts) &&
               checkRoyalty (rAddr nfts) (nPrice nfts) &&
               checkSingleBuy
  ...
  where
    ...
    checkRoyalty :: [RoyAddr] -> Integer -> Bool
    checkRoyalty [] _ = True 
    checkRoyalty (x:xs) price = ((nRoyPrct x == 0) || (fromInteger (Ada.getLovelace (Ada.fromValue (valuePaidTo info $ nRoyAddr x))) >= (nRoyPrct x) % 1000 * fromInteger price)) && checkRoyalty xs price
