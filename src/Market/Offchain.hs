...
import PlutusTx.Prelude as Plutus
    ( Bool( True ),return, Bool, Maybe(..), Eq((==)), (<$>), ($), Integer, (++), (+), isJust, map )
import Ledger
    ( scriptAddress,
      pubKeyHash,
      getCardanoTxId,
...
startSale sp = do
    ...
    let val     = Value.singleton (sCs sp) (sTn sp) 1
        nfts    = NFTSale { nSeller = pkh, nToken = sTn sp, nCurrency = sCs sp, nPrice = sPrice sp, rAddr =  sAd sp }
    ...
    case sale of
        ...
        Just (oref, o, nfts) -> do
            let r       = Redeemer $ PlutusTx.toBuiltinData Buy
                ...
                valAdaS = Ada.lovelaceValueOf (ceiling ((1 - 0.02 - (fromIntegral (sumRoyalty $ rAddr nfts) / 100)) Prelude.* fromIntegral (nPrice nfts) :: Float))
            ...
            if (sumRoyalty $ rAddr nfts) == 0 then do
