...
data RoyAddr = RoyAddr
    { 
    nRoyAddr   :: !PubKeyHash
    , nRoyPrct   :: !Plutus.Integer
    } deriving (Pr.Eq, Pr.Ord, Show, Generic, ToJSON, FromJSON, ToSchema)

instance Eq RoyAddr where
    {-# INLINABLE (==) #-}
    b == c = (nRoyAddr b == nRoyAddr c) &&
             (nRoyPrct b == nRoyPrct c)

PlutusTx.unstableMakeIsData ''RoyAddr
PlutusTx.makeLift ''RoyAddr
